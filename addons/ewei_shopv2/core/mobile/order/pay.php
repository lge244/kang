...
class Pay_EweiShopV2Page extends MobileLoginPage
{
    public function main()
    {
        ...
        if( !empty($ispeerpay) )
        {
            if( fpdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename("ewei_shop_order_peerpay_payinfo") . " WHERE openid = :openid AND pid = :pid", array( ":openid" => $_W["openid"], ":pid" => $ispeerpay["id"] )) )
            {
...
//		}
    ...
    public function complete()
    {
        ...
        if( $type == "credit" )
        {
            ...
            pdo_update('ewei_shop_member', ['today_price +=' => $order['price'], 'sum_price +=' => $order['price']], ['openid' => $openid]);

            //支付升级
            $level = pdo_getall("ewei_shop_member_level",array('uniacid'=>5),array('levelname','ordermoney','id','level'));
            $price = pdo_get('ewei_shop_member',array('openid'=>'wap_user_5_15756002958'),array('sum_price'));

            $levels = 0;
            foreach ($level as $kl=>$vl){
                if($price['sum_price'] > $vl['ordermoney']){
                    $levels =$vl['level'];
                }
            }
            pdo_update("ewei_shop_member",array('level'=>$levels),array('id',$member['id']));
            ...
            if(!$item1['wholesale']) {
                ...
                if (!empty($pagency)) {
                  $ppid =  pdo_getall("ewei_shop_member", array('is_agency' => $pagency['id']), array('id'));
                  foreach($ppid as $kp=>$vp){
                    pdo_insert("ewei_shop_commission_list", array('uid' => $member['id'], 'getcomid' => $vp['id'], 'commission' => $order['price'] * $pagency['proportion'] * 0.01* (1-0.12) / count($pagency), 'status' => 7, 'create_time' => time()));
                    $credit = $cr['credit2'] + $order['price'] * $pagency['proportion'] * 0.01* (1-0.12) / count($ppid);
                    pdo_update("ewei_shop_member", array('credit2' => $credit), array('id' => $vp['id']));
                  }      
                }
                ...
                if (!empty($cagency)) {
                    $pcid = pdo_get("ewei_shop_member", array('is_agency' => $cagency['id']), array('id'));
                  	foreach($pcid as $kc=>$vc){
                        pdo_insert("ewei_shop_commission_list", array('uid' => $member['id'], 'getcomid' => $vc['id'], 'commission' => $order['price'] * $cagency['proportion'] * 0.01* (1-0.12) / count($pcid), 'status' => 7, 'create_time' => time()));
                        $credit = $cr['credit2'] + $order['price'] * $cagency['proportion'] * 0.01* (1-0.12) / count($pcid);
                        pdo_update("ewei_shop_member", array('credit2' => $credit), array('id' => $vc['id']));
                    }
                }
                ...
                if (!empty($aagency)) {
                    ...
                    foreach($paid as $ka=>$va){
                      pdo_insert("ewei_shop_commission_list", array('uid' => $member['id'], 'getcomid' => $va['id'], 'commission' => $order['price'] * $aagency['proportion'] * 0.01* (1-0.12) / count($paid), 'status' => 7, 'create_time' => time()));
                      $credit = $cr['credit2'] + $order['price'] * $aagency['proportion'] * 0.01* (1-0.12) / count($paid);
                      pdo_update("ewei_shop_member", array('credit2' => $credit), array('id' => $va['id']));
                    }
                }
            } else {
                $pagency = pdo_get("agency_area",array('address'=>$province),array('id','proportion'));
                ...
                if(!empty($pagency)){
                    ...
                    foreach($ppid as $kp=>$vp){
                      pdo_insert("ewei_shop_commission_list",array('uid'=>$member['id'],'getcomid'=>$vp['id'],'commission'=>$order['price'] * $pagency['proportion'] * 0.01* (1-0.12) *2 / count($ppid),'status'=>7,'create_time'=>time()));
                      $credit = $cr['credit2'] + $order['price'] * $pagency['proportion']* 0.01 * (1-0.12) *2 / count($ppid);
                      pdo_update("ewei_shop_member",array('credit2'=>$credit),array('id'=>$vp['id']));
                    }
                }
                $cagency = pdo_get("agency_area",array('address'=>$city),array('id','proportion'));
                if(!empty($cagency)){
                    $pcid = pdo_get("ewei_shop_member",array('is_agency'=>$cagency['id']),array('id'));
                     foreach($pcid as $kc=>$vc){
                      pdo_insert("ewei_shop_commission_list",array('uid'=>$member['id'],'getcomid'=>$vc['id'],'commission'=>$order['price'] * $cagency['proportion']* 0.01* (1-0.12) *2 / count($pcid),'status'=>7,'create_time'=>time()));
                      $credit = $cr['credit2'] + $order['price'] * $cagency['proportion']* 0.01 * (1-0.12) *2 / count($pcid);
                      pdo_update("ewei_shop_member",array('credit2'=>$credit),array('id'=>$vc['id']));
                     }
                }
                $aagency = pdo_get("agency_area",array('address'=>$area),array('id','proportion'));
                if(!empty($aagency)){
                    $paid = pdo_get("ewei_shop_member",array('is_agency'=>$aagency['id']),array('id'));
                     foreach($paid as $ka=>$va){
                     	pdo_insert("ewei_shop_commission_list",array('uid'=>$member['id'],'getcomid'=>$va['id'],'commission'=>$order['price'] * $aagency['proportion']* 0.01* (1-0.12) *2 / count($paid),'status'=>7,'create_time'=>time()));
                        $credit = $cr['credit2'] + $order['price'] * $aagency['proportion']* 0.01* (1-0.12) *2 / count($paid);
                        pdo_update("ewei_shop_member",array('credit2'=>$credit ),array('id'=>$va['id']));
                     }
                }
                if($member['fid'] != 0){
                    pdo_insert("ewei_shop_commission_list",array('uid'=>$member['id'],'getcomid'=>$member['fid'],'commission'=>$order['price'] * 0.1* (1-0.12),'status'=>6,'create_time'=>time()));
                    $scale = pdo_get("ewei_shop_commission_scale",array('id'=>18),array('scale'));
                    $credit = $cr['credit2'] + $order['price'] * $scale['scale'] * 0.01 * (1-0.12);
            ...
            if($item1['promotion']){
                if($member['fid'] != 0){
                    $scale1 = pdo_get("ewei_shop_commission_scale",array('id'=>19),array('scale'));
                    $scale2 = pdo_get("ewei_shop_commission_scale",array('id'=>20),array('scale'));

                    $cr = pdo_get("ewei_shop_member",array('id'=>$member['fid']),array('credit2','is_league'));
                    if($cr['is_league']){
                        $credit = $cr['credit2'] + $order['price'] * $scale2['scale'] * 0.01 * (1-0.12);
                    }else{
                        $credit = $cr['credit2'] + $order['price'] * $scale1['scale'] * 0.01 * (1-0.12);
                    }
                    pdo_insert("ewei_shop_commission_list",array('uid'=>$member['id'],'getcomid'=>$member['fid'],'commission'=>$credit,'status'=>6,'create_time'=>time()));
            ...
            if($shareCommission){
                if($member['fid'] != 0){
                    ...
                    //添加到佣金记录表中
                    pdo_insert("ewei_shop_commission_list",array('uid'=>$member['id'],'getcomid'=>$member['fid'],'commission'=>$shareCommission* (1-0.12)*0.01,'status'=>1,'create_time'=>time()));
                    //修改member表中佣金数据
                    $cr = pdo_get("ewei_shop_member",array('id'=>$member['fid']),array('credit2'));
                    $credit = $cr['credit2'] + $shareCommission * (1-0.12)*0.01;
                    pdo_update("ewei_shop_member",array('credit2'=>$credit),array('id'=>$member['fid']));
                }
            }
            //管理2代
            if($manageCommission['grade2Commission']){
                ...
                pdo_insert("ewei_shop_commission_list",array('uid'=>$member['id'],'getcomid'=>$uid['fid'],'commission'=>$manageCommission['grade2Commission']* (1-0.12)*0.01,'status'=>2,'create_time'=>time()));
                //修改member表中佣金数据
                $cr = pdo_get("ewei_shop_member",array('id'=>$uid['fid']),array('credit2'));
                $credit = $cr['credit2'] + $manageCommission['grade2Commission'] * (1-0.12)*0.01;
            ...
            if($manageCommission['grade3Commission']){
                ...
                pdo_insert("ewei_shop_commission_list",array('uid'=>$member['id'],'getcomid'=>$u3id['fid'],'commission'=>$manageCommission['grade3Commission']* (1-0.12)*0.01,'status'=>2,'create_time'=>time()));
                //修改member表中佣金数据
                $cr = pdo_get("ewei_shop_member",array('id'=>$u3id['fid']),array('credit2'));
                $credit = $cr['credit2'] + $manageCommission['grade3Commission'] * (1-0.12)*0.01;
                pdo_update("ewei_shop_member",array('credit2'=>$credit),array('id'=>$u3id['fid']));
            }

            //推广奖
            $tid = m("share")->pushCommission($member['fid']);
			
            if($tid) {
                $scale = pdo_get("ewei_shop_commission_scale", array('id' => 7), array('scale'));
				
                $pushCommission = $order['price'] * $scale['scale'] * 0.01 * (1 - 0.12);
				
              	pdo_insert("ewei_shop_commission_list",array('uid'=>$member['id'],'getcomid'=>$tid,'commission'=>$pushCommission,'status'=>8,'create_time'=>time()));
                $cr = pdo_get("ewei_shop_member", array('id' => $tid), array('credit2'));

                $credit = $cr['credit2'] + $pushCommission;
                pdo_update("ewei_shop_member", array('credit2' => $credit), array('id' => $tid));
            }
